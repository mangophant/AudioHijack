...
class CoreAttention(torch.nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        return context_layer, attention_probs.reshape(output_size[0], output_size[1], output_size[2], -1)
...
class SelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        context_layer, attention_weights = self.core_attention(query_layer, key_layer, value_layer, attention_mask)
        ...
        return output, kv_cache, attention_weights
...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        # Self attention.
        attention_output, kv_cache, attention_weights = self.self_attention(
            layernorm_output,
            attention_mask,
            rotary_pos_emb,
            kv_cache=kv_cache,
...
class GLMTransformer(torch.nn.Module):
    ...
    def forward(
        ...
        for index in range(self.num_layers):
            ...
            hidden_states, kv_cache, attention_weights = layer_ret
...
class ChatGLMPreTrainedModel(PreTrainedModel):
    ...
    # TODO [modified]
    def get_masks(self, shape, device, past_key_values, padding_mask=None):
    ...
    def forward(
        ...
        if full_attention_mask is None:
            if (attention_mask is not None and not attention_mask.all()) or (past_key_values and seq_length != 1):
                full_attention_mask = self.get_masks(inputs_embeds.shape[:-1], inputs_embeds.device, past_key_values, padding_mask=attention_mask)
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel, GenerationMixin):
    ...
    def _update_model_kwargs_for_generation(
        ...
        # TODO: [modified] generation() of transformers==4.53.0 does not support _extract_past_from_model_output
