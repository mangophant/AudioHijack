...
class QWenTokenizer(PreTrainedTokenizer):
    ...
    def process_audio_data(self, audio):
        ...
        audio_len_after_cnn = get_T_after_cnn(mel_len)
        audio_token_num = (audio_len_after_cnn - 2) // 2 + 1
        input_audio_lengths = torch.tensor([[audio_len_after_cnn, audio_token_num]], 
            dtype=torch.int, device=audio.device)
        audio_span_tokens = [audio_token_num + 2] # add audio bos eos
        return {"input_audios": mel.unsqueeze(0),
                "input_audio_lengths": input_audio_lengths,
                "audio_span_tokens": audio_span_tokens}
